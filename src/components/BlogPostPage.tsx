...
export default function BlogPostPage({ post, lang = 'it', translationFallback = false }: Props) {
  const t = dict[lang];
  return (
    <div className="t-bg t-text overflow-x-hidden" style={{ fontFamily: 'GraphikLCG, Inter, system-ui, sans-serif' }}>
      ...
      <section className="max-w-[900px] mx-auto px-5 md:px-12 pt-36 md:pt-44 pb-10 md:pb-14">
        {translationFallback && (
          <div className="mb-8 px-5 py-3 rounded-2xl text-sm" style={{ background: 'rgba(255,106,0,0.08)', border: '1px solid rgba(255,106,0,0.25)', color: 'rgba(var(--c-text),0.7)' }}>
        ...
        <div className="space-y-6">
          ...
          <FadeIn>
            <div className="flex items-center gap-4 flex-wrap">
              <a href={withLang('/blog', lang)} className="flex items-center gap-2 text-xs uppercase tracking-widest transition-colors duration-200 hover:text-[#FF6A00]" style={{ color: 'rgba(var(--c-text),0.4)' }}>
          ...
          <FadeIn delay={0.18}>
            <div className="flex items-center gap-4 text-sm" style={{ color: 'rgba(var(--c-text),0.35)' }}>
              ...
              <span>·</span>
              <span>{post.read_time} {t.blogPost.minRead}</span>
            </div>
          </FadeIn>
        </div>
      </section>
      ...
      <section className="max-w-[750px] mx-auto px-5 md:px-12 pb-24 md:pb-32">
        ...
        <FadeIn>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'rgba(var(--c-text),0.3)' }}>{t.blogPost.writtenBy}</p>
              <p className="font-semibold t-text">{post.author}</p>
              <p className="text-sm mt-1" style={{ color: 'rgba(var(--c-text),0.4)' }}>{t.blogPost.agencyLine} — Vittorio Veneto (TV)</p>
            </div>
            <a
      ...
      <section className="max-w-[1400px] mx-auto px-5 md:px-12 pb-24">
        <FadeIn>
          <div className="t-card t-border border rounded-[2rem] p-10 md:p-14 text-center space-y-5">
            <p className="text-xs text-[#FF6A00] uppercase tracking-widest font-semibold">{t.blogPost.continueReading}</p>
            <h2 className="font-cal font-semibold uppercase tracking-tighter italic t-text text-[clamp(1.8rem,4vw,3rem)] leading-tight">
              {t.blogPost.backTitlePrefix} <span className="text-[#FF6A00]">{t.blogPost.backTitleHighlight}</span>
            </h2>
